...
public class LegacyConverter {
	StructureEntry convertToTemplate(HolderGetter<Block> blockLookup, ParsedStructure structure, String templatePath) {
		...
		try {
			String fullPathStr = templatePath + ".nbt";
			Path fullPath = Path.of(fullPathStr);
			String fileName = fullPath.getFileName().toString();
			File folder = new File(fullPathStr.substring(0, fullPathStr.length() - fileName.length()));
			if (!folder.exists()) {
				folder.mkdirs();
			}
			NbtIo.writeCompressed(writer.buildResult(), fullPath);
		} catch (IOException e) {
