...
public class SpawnerSpawnGroup {
	...
	public AdvancedSpawnerSettings.SpawnGroup save() {
		...
		for (Entry e : entitiesToSpawn) {
			AdvancedSpawnerSettings.SpawnEntry entry = new AdvancedSpawnerSettings.SpawnEntry(e.minToSpawn, e.maxToSpawn, e.remainingSpawnCount, e.entityId);
			entries.add(entry);
		}
	...
	public static class Entry {
		...
		public void spawnEntities(ServerLevel world, BlockPos spawnPos, int range) {
			...
			for (int i = 0; i < toSpawn; i++) {
				...
				Entity e = type.create(world, EntitySpawnReason.SPAWNER);
